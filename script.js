document.addEventListener("DOMContentLoaded", function() {
    ...
    stasiunObject.addEventListener("load", function() {
        ...
        const signals = {
            ...
            S1_langsir: { light: svgDoc.getElementById("S1_badug"), name: "Langsir 1" }, // ID Disesuaikan dengan XML
            S2_langsir: { light: svgDoc.getElementById("S2_badug"), name: "Langsir 2" }  // ID Disesuaikan dengan XML
        };
        const wesels = {
        ...
        function createButtons() {
            controlPanel.innerHTML = `
                <div class="button-group">
                    ...
                    <button id="btn-toggle-S1_langsir" class="btn-signal-langsir"><i class="fa-solid fa-arrow-down"></i> Langsir 1</button>
                    <button id="btn-toggle-S2_langsir" class="btn-signal-langsir"><i class="fa-solid fa-arrow-down"></i> Langsir 2</button>
                </div>
                <div class="button-group">
        ...
        function attachEventListeners() {
            ...
            buttons.keretaMasuk.addEventListener('click', () => {
                if (gameState !== 'IDLE' || wesel1Posisi !== 'J2&J3' || wesel2Posisi !== 'J2' || !checkSignal('S_in')) {
                    logActivity("GAGAL: Syarat kereta masuk tidak terpenuhi! (Wesel harus ke Jalur 2 & Sinyal Masuk harus hijau)", true); return;
                }
                logActivity("Kereta mulai masuk ke Jalur 2..."); gameState = 'ANIMATING'; updateTombol();
                train.style.visibility = "visible";
                animateElement(train, rute.masuk_J2, 10, () => {
                    setSignal('S_in', 'red');
                    gameState = 'ARRIVED_ON_J2'; logActivity("Kereta telah tiba di Jalur 2."); updateTombol();
                });
            });

            buttons.lepasLoko.addEventListener('click', () => {
                if (gameState !== 'ARRIVED_ON_J2' || !checkSignal('S2_langsir')) {
                    logActivity("GAGAL: Syarat lepas lokomotif tidak terpenuhi! (Sinyal Langsir 2 harus hijau)", true); return;
                }
            ...
            buttons.langsirViaJ1.addEventListener('click', () => {
                if (gameState !== 'LOCO_AT_BADUG' || wesel1Posisi !== 'J1' || !checkSignal('S1_langsir') || !checkSignal('S1_out')) {
                    logActivity("GAGAL: Syarat langsir via Jalur 1 tidak terpenuhi! (Wesel 1 ke J1, Sinyal Langsir 1 & S1 Keluar harus hijau)", true); return;
                }
            ...
            buttons.kembaliKeRangkaian.addEventListener('click', () => {
                if (gameState !== 'LOCO_SHUNTED' || wesel1Posisi !== 'J2&J3' || wesel2Posisi !== 'J2' || !checkSignal('S_in')) {
                    logActivity("GAGAL: Syarat lokomotif kembali tidak terpenuhi! (Wesel harus ke Jalur 2 & Sinyal Masuk harus hijau)", true); return;
                }
            ...
            buttons.berangkatkanKereta.addEventListener('click', () => {
                if (gameState !== 'READY_TO_DEPART' || wesel1Posisi !== 'J2&J3' || wesel2Posisi !== 'J2' || !checkSignal('S2_out')) {
                    logActivity("GAGAL: Syarat keberangkatan tidak terpenuhi! (Wesel harus ke Jalur 2 & Sinyal S2 Keluar harus hijau)", true); return;
                }
